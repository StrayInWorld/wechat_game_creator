...
cc.Class({
    ...
    start () {
        ...
        var targetX=cc.p(-canvasWidth/2+nodeWidth/2,this.node.y)
        if(!this.isLeft){
            targetX=cc.p(canvasWidth/2-nodeWidth/2,this.node.y);
        }
        this.node.runAction(cc.moveTo(1.5,targetX).easing(cc.easeBounceInOut(0.5)));
